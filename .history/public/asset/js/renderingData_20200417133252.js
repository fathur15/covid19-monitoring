...
function renderPerkembangan(data, tipe='line'){
    ...
    keys.forEach(el => {
        arrayOfData.totalCase.push(data[el].total_cases);
        arrayOfData.dateTime.push(data[el].total_cases);
    });
    if(data){
...
function renderRealTimeData(){
    ...
    renderPerkembangan(dataPerkembangan.responseJSON.stat_by_countr);
}
renderRealTimeData();
